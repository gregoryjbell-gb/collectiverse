import { NextRequest, NextResponse } from 'next/server';
import { getSession, ensureUserId } from '@/lib/auth';
import { prisma } from '@/lib/prisma';
import { saveInventoryImageDerivatives } from '@/lib/image-processing';
import { mkdir } from 'fs/promises';
import { join } from 'path';

const MAX_SIZE = 10 * 1024 * 1024;
const ALLOWED_TYPES = ['image/jpeg', 'image/png', 'image/webp'];
const FIELD_MAP: Record<string, string> = {
  frontScan: 'frontImageUrl',
  backScan: 'backImageUrl',
  privateImage: 'privateImageUrl',
};
const FIELD_TYPE_MAP: Record<string, 'front' | 'back' | 'private'> = {
  frontScan: 'front',
  backScan: 'back',
  privateImage: 'private',
};

export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

  let userId: string;
  try { userId = await ensureUserId(session); } catch { return NextResponse.json({ error: 'Unauthorized' }, { status: 401 }); }

  const group = await (prisma as any).inventoryGroup.findFirst({ where: { id: params.id, userId } });
  if (!group) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const user = await prisma.user.findUnique({ where: { id: userId }, select: { displayName: true, username: true } });
  const watermarkText = `${user?.displayName || user?.username || 'User'} • Collectiverse`;

  const formData = await req.formData();
  const updateData: Record<string, string> = {};
  const urls: Record<string, string> = {};

  // Use groups-specific storage path
  const groupDir = join(process.cwd(), 'storage', 'private', 'users', userId, 'groups', params.id);
  await mkdir(groupDir, { recursive: true });

  for (const [fieldName, dbField] of Object.entries(FIELD_MAP)) {
    const file = formData.get(fieldName);
    if (!file || !(file instanceof File) || file.size === 0) continue;

    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ error: `Invalid file type for ${fieldName}. Allowed: JPEG, PNG, WebP` }, { status: 400 });
    }
    if (file.size > MAX_SIZE) {
      return NextResponse.json({ error: `${fieldName} exceeds 10 MB limit` }, { status: 400 });
    }

    const buffer = Buffer.from(await file.arrayBuffer());
    const imageType = FIELD_TYPE_MAP[fieldName];

    // Reuse the image processing pipeline but store in groups folder
    const displayFilename = await saveGroupImageDerivatives({
      userId,
      groupId: params.id,
      type: imageType,
      buffer,
      watermarkText,
    });

    updateData[dbField] = displayFilename;
    urls[fieldName] = `/api/inventory-groups/${params.id}/media/${imageType}`;
  }

  if (Object.keys(updateData).length === 0) {
    return NextResponse.json({ error: 'No valid files uploaded' }, { status: 400 });
  }

  await (prisma as any).inventoryGroup.update({ where: { id: params.id }, data: updateData });
  return NextResponse.json({ urls, message: 'Upload successful' });
}

// Group-specific image derivative saver
import { writeFile } from 'fs/promises';
import { normalizeImage, createWatermarkedDisplayImage, createWatermarkedThumbnail } from '@/lib/image-processing';

async function saveGroupImageDerivatives(opts: {
  userId: string;
  groupId: string;
  type: 'front' | 'back' | 'private';
  buffer: Buffer;
  watermarkText: string;
}): Promise<string> {
  const { userId, groupId, type, buffer, watermarkText } = opts;
  const dir = join(process.cwd(), 'storage', 'private', 'users', userId, 'groups', groupId);
  await mkdir(dir, { recursive: true });

  const normalized = await normalizeImage(buffer);
  await writeFile(join(dir, `${type}-original.webp`), normalized);

  const display = await createWatermarkedDisplayImage(normalized, watermarkText);
  await writeFile(join(dir, `${type}-display.webp`), display);

  const thumb = await createWatermarkedThumbnail(normalized, watermarkText);
  await writeFile(join(dir, `${type}-thumb.webp`), thumb);

  return `${type}-display.webp`;
}
