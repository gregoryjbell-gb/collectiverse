...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await (prisma as any).liveBreak.update({ where: { id: params.id }, data: { status: 'COMPLETED' } });

  await (prisma as any).liveEventMessage.create({
    data: { liveEventId: liveBreak.liveEventId, messageType: 'SYSTEM', message: `✅ Break completed: "${liveBreak.title}"` },
  });
